...
### 📐 다중 도메인 연동 및 참조 무결성을 고려한 ERD 분석
대시보드에 표현되는 포인트, 결제, 출결, 수강 이력 등 방대한 개인화 데이터를 정합성 있게 다루고, 회원 탈퇴 시 복잡하게 얽힌 자식 테이블들의 연쇄 삭제(Cascade) 및 참조 무결성을 안전하게 보장하기 위해 전체 도메인 관계를 면밀히 분석하여 개발을 진행했습니다.

<p align="center">
  <img src="./images/module2ERD.png" width="90%" alt="모듈2 참조 ERD"><br>
  <sub>▲ 모듈 2 ERD: 사용자의 입장에서 고려하여 마이페이지 내 최대한 많은 정보 제공(7/13 테이블 참조)</sub>
</p>

* 다수의 테이블을 참조한 만큼 전반적인 도메인 이해도를 가지고 시나리오 작성 및 더미 데이터 생성
...
* **연쇄 삭제(Cascade) 처리**: JPA 및 DB 제약 조건을 고려하여 자식 테이블(댓글, 제출 문제, 게시글, 수강 내역, 섹션 등)의 데이터를 순차적으로 삭제하여 참조 무결성 오류 해결.
