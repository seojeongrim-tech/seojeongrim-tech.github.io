# ⚙️ 친구 및  1:1 메시징 시스템 구축기 (Module 3)
> **핵심 요약:** DDD 기반 친구/메시지 정책 수립, 다중 조건 조회(AND/OR 우선순위) 트러블슈팅, 유령 방 및 맞요청 버그 차단, 재입장 사용자의 타임라인 격리 알고리즘 구현

---

## 1. 개요
> 모듈 1에서는 메시지 기능을 반복문 기반의 단순 조회 방식으로 구현했습니다. 모듈 3에서 팀 차원에서 웹소켓(STOMP) 기반 실시간 아키텍처를 도입하기로 결정한 가운데, 이전 프로젝트에서 다뤘던 메시지 도메인을 다시 맡아 이어서 발전시키고자 지원했습니다.
...
### 2-4. 개인 로깅 전략 수립 및 AI 기반 디버깅 가속화
* **선제적 로그 설계**: 팀원들이 PR을 확인하지 않아 콘솔 출력에만 의존해야 하는 환경에서, 예외 발생 지점·상태 전이 지점 등 유의미한 구간에 직접 로그를 심어 문제 원인을 스스로 추적할 수 있는 체계를 마련했습니다.
* **로그 수집 설정을 통한 AI 협업 효율화**: 로그 수집 설정을 구성해, 에러 발생 시 관련 로그만 선별 추출하여 AI에게 전달하는 방식으로 트러블슈팅 속도를 개선했습니다. 이를 통해 4번 섹션의 트러블슈팅 사례들을 더 빠르게 특정하고 해결할 수 있었습니다.
