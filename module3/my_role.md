...
* **담당 역할**: 복잡한 친구 및 1:1 메시지 도메인 기능 개발 & 프로젝트 DBA 역할 수행
* **협업 도구**: GitHub, Slack, Notion
* **핵심 타깃**: 프론트엔드 개발자가 데이터를 소비하는 과정에서의 불편함을 줄이고, 데이터 정합성을 최적화하는 API 설계
...
### 👥 친구 목록 및 사용자 검색 관리
* **수강 신청 기반 자동 연동**: 사용자가 수강 신청을 진행하면 비동기 이벤트를 통해 `friend` 테이블에 강사-학생 관계가 자동으로 추가되는 백인프라 구축. 이미 해당 강사의 과목을 수강하고 있다면 친구 관계이므로 두 사용자의 친구 관계를 오직 한 행으로 처리하기 위해 `friend` 테이블에 행이 존재하면 강사-학생 자동 친구 로직을 건너뛰도록 처리하여 데이터 정합성을 지키고 불필요한 행(Row) 누적을 원천 차단.
...
* **차단 목록 주체 식별**: 친구 차단 시 `fromUserId`를 차단 주체로 명확히 명시하여, 상대방이 나를 차단한 내역은 내 차단 목록에 노출되지 않도록 데이터 격리 성공. 두 사용자의 친구 관계를 오직 한 행으로 처리하기 위해 차단 시 `fromUserId`를 로그인한 사용자로 업데이트하여 프라이버시 및 정합성 보장.
...
> * ** 상대방이 이미 요청을 보낸 상태에서 맞요청 시의 예외 발생 상황입니다. **
...
<p align="center">
  <img src="./images/module3ERD.png" width="90%" alt="모듈3 참조 ERD"><br>
  <sub>▲ 모듈 3 참조 ERD: 파편화되고 문법 오류가 있던 초기 데이터 모델을 분석하여 친구, 메시지, 알림 테이블 간의 외래키(FK) 및 정합성 기준을 재정립(10/25 테이블 참조)</sub>
</p>
