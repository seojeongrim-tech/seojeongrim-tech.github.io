...
### 👥 친구 목록 및 사용자 검색 관리
* **수강 신청 기반 자동 연동**: 사용자가 수강 신청을 진행하면 비동기 이벤트를 통해 `friend` 테이블에 강사-학생 관계가 자동으로 추가되는 백인프라 구축. 이미 해당 강사의 과목을 수강하고 있다면 친구 관계이므로 `friend` 테이블에 행이 존재하면 강사-학생 자동 친구 로직 건너뛰기를 구현하여 무의미한 데이터가 추가되는 정합성 고려.
