...
1. **Request**: 클라이언트가 HTTP POST `/api/v1/messages/send/{roomId}`로 메시지 요청을 보냅니다.
...
3. **Processing**: DB에 메시지를 안전하게 저장한 뒤, 비동기 이벤트를 발행하여 `notification` 테이블에 `"{nickname}님이 메시지를 전송했습니다."` 행을 추가 또는 업데이트(알림 내용, 시간, 읽음 여부 갱신)합니다.
4. **WebSocket Publish**: `SimpMessagingTemplate`을 활용해 `/sub/chat/room/{roomId}` 채널로 메시지 객체를 방출(Publish)합니다.
5. **Response**: 해당 방을 구독(Subscribe) 중인 상대방 화면에 실시간으로 메시지가 렌더링되도록 합니다.
...
### 🔗 소스 코드 확인
* [LMS 친구 & 메시징 프로젝트 전체 소스 코드 (GitHub)](https://github.com/seojeongrim-tech/Module07-jpa-mini-project)
